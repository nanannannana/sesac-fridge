  ...
  if (thisFridge == "fresh") {
    Swal.fire({
      ...
      preConfirm: () => {
        ...
        const freshRange =
          Swal.getPopup().querySelector("#freshRange_inp").value;
        const freshExpire =
    ...
    Swal.fire({
      ...
      preConfirm: () => {
        ...
        const frozenDate =
          Swal.getPopup().querySelector("#frozenDate_inp").value;
...
function updateFrozen(result) {
  axios({
    method: "patch",
    url: "/api/v1/fridge/updateFrozen",
    data: {
      name: result.value.name,
      range: result.value.range,
